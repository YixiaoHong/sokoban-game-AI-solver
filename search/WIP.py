...
def checkBorderLock(state):  # Check whether there is a box near border but no avaliable storage left for it on this side
    ...
    for i in state.storage:
        if i not in state.boxes:
            avaliableStorage.append(i)
        ...
        if box not in state.storage:
            ...
            if left1(box, state) == False:
                for j in range(state.height):
                    if (0, j) in avaliableStorage:
                        checkAvaliable = True
                if checkAvaliable != True:
                    return True

            if right1(box, state) == False:
                for j in range(state.height):
                    if (state.width - 1, j) in avaliableStorage:
                        checkAvaliable = True
                if checkAvaliable != True:
                    return True
            ...
            if down1(box, state) == False:
                for i in range(state.height):
                    if (i, state.width) in avaliableStorage:
                        checkAvaliable = True
                if checkAvaliable != True:
                    return True
...
###############################################################
#Test OK
def returnBoxInStorgae(state):
    boxInStorage = set()
    for box in state.boxes:
        if box in state.storage:
            boxInStorage.add(box)

    return boxInStorage

def anyBoxNearRobot(state,i):
    boxNearRobot = set()
    # print("check robot:",i)
    if left1(i, state):
        if left1(i, state) in state.boxes:
            # print("L boxes")
            boxNearRobot.add(i)
    if right1(i, state):
        if right1(i, state) in state.boxes:
            # print("R boxes")
            boxNearRobot.add(i)
    if up1(i, state):
        if up1(i, state) in state.boxes:
            # print("U boxes")
            boxNearRobot.add(i)
        if left1(up1(i, state), state):
            if left1(up1(i, state), state) in state.boxes:
                # print("UL boxes")
                boxNearRobot.add(i)
        if right1(up1(i, state), state):
            if right1(up1(i, state), state) in state.boxes:
                # print("UR boxes")
                boxNearRobot.add(i)
    if down1(i, state):
        if down1(i, state) in state.boxes:
            # print("D boxes")
            boxNearRobot.add(i)
        if left1(down1(i, state), state):
            if left1(down1(i, state), state) in state.boxes:
                # print("DL boxes")
                boxNearRobot.add(i)
        if right1(down1(i, state), state):
            if right1(down1(i, state), state) in state.boxes:
                # print("DR boxes")
                boxNearRobot.add(i)
    # print(boxNearRobot)
    if boxNearRobot != set():
...
def robotHvalForCornerCase(state,farthestBox):
    totalRobotManhattanDis = 0
    for i in state.robots:
        if anyBoxNearRobot(state,i) != True:
            temprobotManhattanDis = abs(i[0] - farthestBox[0]) + abs(i[1] - farthestBox[1])
            if i in state.storage:
                temprobotManhattanDis = temprobotManhattanDis + 2
            totalRobotManhattanDis = totalRobotManhattanDis + temprobotManhattanDis
        else:
            continue
    return totalRobotManhattanDis

def boxNotAlong(state):
    boxNearRobot = set()
    for i in state.boxes:
        if i in state.storage:
            #print("check box:",i)
...
def corner_consentrated_storage_h_f(state,corner):
    ...
    if corner == 'UL':
        for i in state.boxes:
            ...
            if boxManhattanDis > farthestBoxDis:
                farthestBoxDis = boxManhattanDis
                farthestBox = i
            totalboxManhattanDis = totalboxManhattanDis + boxManhattanDis

        h_val = totalboxManhattanDis**2 + robotHvalForCornerCase(state,farthestBox)+0*boxNotAlong(state)
        ...
        for i in state.boxes:
            ...
            if boxManhattanDis > farthestBoxDis:
                ...
                farthestBox = i
            totalboxManhattanDis = totalboxManhattanDis + boxManhattanDis
        h_val = totalboxManhattanDis + robotHvalForCornerCase(state,farthestBox)
        ...
        for i in state.boxes:
            ...
            if boxManhattanDis > farthestBoxDis:
                ...
                farthestBox = i
            totalboxManhattanDis = totalboxManhattanDis + boxManhattanDis
        h_val = h_val = totalboxManhattanDis**2 + 0*robotHvalForCornerCase(state,farthestBox)
    ...
    if state.parent:
        parentBox = set()
        childBox = set()
        parentBoxInStorage = set()
        childBoxInStorgage = set()
        for p in state.parent.boxes:
            parentBox.add(p)
            if p in state.storage:
                parentBoxInStorage.add(p)
        for c in state.boxes:
            childBox.add(c)
            if c in state.boxes:
                childBoxInStorgage.add(c)
        if childBox == parentBox:
            childMovedBox = False
        else:
            childMovedBox = True

    if childMovedBox == True:
        # print("Child Moved Box")
        h_val = h_val - 2
    """
    return h_val
...
def anytime_weighted_astar(initial_state, heur_fn, weight=1., timebound = 10):
    ...
    while currentTime < stopTime:
        ...
        if iteration !=0:
            weight = 1 + (0.9 ** iteration) * (initialWeight - 1)                                                       #Decrease the amont of weight exceed 1 with 10% comparing to last iteration
        ...
        if solution != False:                                                                                           # If solution find before time limit
            print("new gval is", solution.gval, "indes is ", solution.index)
...
a= SokobanState("START", 0, None, 6, 6, # dimensions
                 ( (0, 2), (0, 4)), #robots
                 frozenset(((1, 0), (5, 3), (0, 1), (4, 3), (5, 5), (4, 0))), #boxes
